...
```{r}
# load in this week's data
this_week = read.csv("model_prepped_this_week.csv")[,-1]
```

```{r}

# current season and week
current_season = max(this_week$season)
current_week = max(this_week$week)
...
# function to load
load_bet_data_func = function(year_start, year_end) {
  
  # loop to load data
  for (szn in c(year_start:year_end)){
    # this one creates the df for the first year in the loop
    if (szn == year_start){
      full_df = cfbd_betting_lines(year = szn, season_type = "regular")
...
# sort providers in terms of what to take
...
```{r}
# create current home line df for t1_team
...
# create current away line df for t1_team
...
```{r}
# write data back out
write.csv.csv(bet_lines_joined, "model_prepped_this_week.csv")[,-1]
```
